...
Updated 2026-09-10 from the current working tree. `docs/GDD.md` is the design authority. `todo` is the exception for explicit task status and priority; where it intentionally differs from the GDD, this plan records the difference instead of silently redefining scope.

Submission deadline: **2026-09-13 13:00 CEST**.

## Current Baseline

The core WebXR game described by the GDD is implemented: five trees, gaze-driven aiming from the tracked headset center, horn-tip rainbow shots, fruit spawning/decay/removal, restoration and rot, wave pacing, restoration-driven ground/flower changes, urgency feedback, XR lifecycle handling, synthesized shot/hit audio, and a win transition.
...
- `npm run lint`: **passed**.
- `npm run build`: **12,014-byte `dist/Unicorn.zip`**, leaving **1,298 bytes** below the 13,312-byte limit.
...
### 1. Make restored trees permanently restored

The GDD requires a fully restored tree to remain permanently restored. `Tree` currently sets `isHealed` once, but direct calls to `hitFruit()` and `rotFruit()` can still change its restoration state afterward. Controller-level guards normally prevent this, but the invariant belongs in `Tree`.

Required outcome:

- Once healed, hit and rot calls no longer change restoration state.
- `tree:healed` is emitted only once.
- Existing completion visuals are not retriggered by later calls.
- Add focused regression coverage for healed-tree immunity and event-once behavior.

### 2. Finish only the audio items still open in TODO
...
Mute remains a Would Like item. Music remains a Could Have item. Neither is a release blocker.

### 3. Sky only if it fits after release-critical work
...
The following open TODO items are nonessential polish and should remain deferred while only 1,298 compressed bytes remain: background environment, music, fruit textures/sprites, tree/grass shader animation, restart button, fruit/entity pooling, fruit-material pooling, better horn, and mute.
...
1. **Permanent healing invariant.** Fix healed-tree immunity in `Tree`, add the focused regressions, then run the regression suite, `npm run lint`, and `npm run build`.
2. **Submission draft.** Upload a known-good production ZIP and repository URL before spending time on optional polish. Prepare the jam-page artwork, captures, story, and hook outside the runtime archive.
3. **Headset release gate.** Test the extracted production build on the target headset: XR entry, tracked gaze aim, visible horn beam interception, hit/rot, pause/resume, tree completion, final win sequence, audio lifecycle, XR exit/re-entry, both-eye readability, comfort, and frame pacing. Check the browser console and production engine URL/version.
4. **Small remaining polish.** If the release gate is healthy and the ZIP has room, add rot/completion sounds first, then the TODO sky improvement. Measure the ZIP after each runtime change. Stop optional work if it threatens the size reserve or headset stability.
5. **Final submission gate.** Build again, verify `dist/Unicorn.zip` is below 13,312 bytes and has top-level `index.html`, test the exact uploaded artifact, confirm the WebXR category/browser requirements, repository URL, title, artwork, story, and live preview, then submit before **2026-09-13 13:00 CEST**.
